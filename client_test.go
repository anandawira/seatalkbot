...
func Test_client_SendGroupMessage(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name        string
		handlerFunc func(http.ResponseWriter, *http.Request)
		...
		{
			name: "it should return messageID and nil error when response body code is 0",
			handlerFunc: func(w http.ResponseWriter, r *http.Request) {
				switch r.URL.Path {
				case "/auth/app_access_token":
					w.WriteHeader(http.StatusOK)
				...
				default:
					w.WriteHeader(http.StatusOK)
					_, _ = w.Write([]byte(`{"code":0,"message_id":"abc"}`))
				}
			},
			checkError: require.NoError,
		},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			messageID, err := c.SendGroupMessage(context.Background(), "123", TextMessage("abc", ""))

			tt.checkError(t, err)
			if err == nil {
				assert.Equal(t, "abc", messageID)
			}
		})
	}
}
